...
const TaskSelect = ({defaultValue, list}) => {
  ...
  return (
    <div className="relative">
      <div
       ...
       px-4 h-10 outline-none focus:border-emerald-500/50 transition flex justify-between items-center
      ...
      <ul
        className={`bg-[#0F0F0F] border border-[#2A2A2A] rounded-lg text-gray-400 text-sm
       transition flex flex-col absolute top-11 left-0 right-0 ${isOpenDropdown ? '' : 'hidden'}`}
      >
        {list.map((item) => (
